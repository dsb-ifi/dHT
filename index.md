...
The key mechanism is the mean-injection formulation. During merging, region features are aggregated up the hierarchy, and once the hierarchy has been pruned, we project these features back to image space with a linear mapping $W$. When $W$ is initialized such that $W f^*(v) = \bar{x}^*(v)$ for every region, the reconstructed image $ \hat{x} $ matches the original input, making the presence of $\partial\text{HT}$ invisible to the ViT. 
